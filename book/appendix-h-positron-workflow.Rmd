...
```{r,echo=FALSE, fig.cap="Positron 的操作流程化处理"}
...
先看 Posit 官方视频 [A quick tour of Positron](https://youtu.be/4Ir_HX4riHw)，配套文字说明见[官方文章](https://posit.co/blog/a-quick-tour-of-positron/)。视频适合熟悉安装后的窗口、项目、编辑器、Console 和数据科学窗格；官方目前没有专门演示 DeepSeek API 配置的视频，所以看完视频后按下面顺序在自己的电脑上操作：

1. 安装 R 和 Positron，打开 `examples/positron-workflow/` 项目；
2. 在 Console 运行 `R.version.string` 和 `getwd()`；
3. 用命令面板运行 `Authentication: Configure Language Model Providers`；
4. 选择 `DeepSeek (Experimental)`，只在凭据输入框粘贴 API Key；
5. 用 `View: Show Posit Assistant` 打开助手，选择当前可用模型；
6. 先发送只读测试问题，再在本地运行 R 代码核对答案。

如果界面没有 DeepSeek，先更新 Positron并查阅[官方服务商文档](https://positron.posit.co/assistant-providers.html)，不要照抄旧教程中的模型名称。
...
本项目附带两个可直接运行的统计写作模板：`paper/templates/arxiv-english/` 是按 arXiv 统计论文常见的 `article + natbib + plainnat` 组织的英文模板，`paper/templates/arxiv-chinese/` 是 `ctexart + natbib + plainnat` 中文模板。两个模板都使用 `references.bib`，正文用作者—年份形式引用。arXiv 没有统一中文模板；arXiv:2412.19437 的源码可作为中文兼容设置的参考，但其 `deepseek.cls` 是定制类，不能当作通用模板。两个模板都用于跑通技术链路，不代表任何期刊格式。拿到真实模板后先复制模板目录，在副本中工作，保留原有 `.cls`、`.sty`、参考文献样式、作者区、章节命令和说明。
...
如已有 LaTeX，在 Terminal 中进入模板目录后编译。英文模板：
...
```sh
cd paper/templates/arxiv-chinese
xelatex -interaction=nonstopmode -halt-on-error main.tex
bibtex main
xelatex -interaction=nonstopmode -halt-on-error main.tex
xelatex -interaction=nonstopmode -halt-on-error main.tex
